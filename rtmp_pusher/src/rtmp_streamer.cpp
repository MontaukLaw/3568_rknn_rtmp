...
static rtmp_client_t *rtmp = nullptr;
static socket_t rtmp_socket = 0; 
...
void rtmp_push_raw_data(int taglen, char *packet)
{
    ...
    uint32_t s_timestamp = 0;
    uint32_t diff = 0;
    uint64_t clock = system_clock(); // timestamp start from 0;
...
void rtmp_init(void)
{
    const char *host = "43.139.145.196";
    const char *app = "live";
    const char *stream = "livestream?secret=e8c13b9687ec47f989d80f08b763fdbf";
    ...
    socket_init();
    rtmp_socket = socket_connect_host(host, 1935, 2000);
    socket_setnonblock(rtmp_socket, 0);

    rtmp_client_t *rtmp = rtmp_client_create(app, stream, packet /*tcurl*/, &rtmp_socket, &handler);
    int r = rtmp_client_start(rtmp, 0);

    while (4 != rtmp_client_getstate(rtmp) && (r = socket_recv(rtmp_socket, packet, sizeof(packet), 0)) > 0)
    {
        assert(0 == rtmp_client_input(rtmp, packet, r));
    }
...
//     rtmp_init();
//     // rtmp_publish_test("video-center.alivecdn.com", "live", "hello?vhost=your.domain", "local-flv-file-name");
//     // rtmp_publish_test("43.139.145.196", "live", "livestream?secret=e8c13b9687ec47f989d80f08b763fdbf", "/userdata/vi_rtmp/out.flv");
// }
